...
class GeminiMelodyGenerator(private val apiKey: String) {
    ...
    companion object {
        private const val TAG = "GeminiMelodyGenerator"
    }

    private val apiClient = GeminiApiClient(apiKey)

    init {
        println("[$TAG] 初期化完了 - APIキー: ${if (apiKey.isEmpty()) "未設定" else "設定済み (${apiKey.length}文字)"}")
    }
    ...
    suspend fun generateMelody(): Result<Score> {
        return try {
            ...
            println("[$TAG] === プロンプト生成完了 ===")
            println("[$TAG] プロンプト長: ${prompt.length}文字")
            println("[$TAG] プロンプト全文:\n$prompt")

            // プロンプトが空または極端に短い場合はデフォルトを補完
            if (prompt.isBlank() || prompt.length < 20) {
                println("[$TAG] 警告: プロンプトが空または極端に短いため、デフォルトプロンプトを使用します")
                prompt = buildDefaultPrompt()
                println("[$TAG] デフォルトプロンプト長: ${prompt.length}文字")
            }
    ...
    private fun buildDefaultPrompt(): String {
        ...
        return """
パッヘルベルのカノンのコード進行に合う美しいメロディを60音符生成してください。
使用可能なピッチ: $availablePitches
JSON配列で出力: [{"pitch": "D5", "duration": 1.0}, ...]
        """.trimIndent()
    }
